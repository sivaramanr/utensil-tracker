const FETCH_LOGGER_FLAG = '__UTENSIL_TRACKER_FETCH_LOGGER_INSTALLED__';
...
function sanitizeHeaders(headersInput) {
  ...
  if (typeof Headers !== 'undefined' && headersInput instanceof Headers) {
    headersInput.forEach((value, key) => {
      ...
      sanitized[key] = SENSITIVE_HEADER_KEYS.has(lowerKey) ? '[REDACTED]' : value;
    });
    return sanitized;
  }
  ...
  for (const key of Object.keys(headersInput)) {
    const lowerKey = key.toLowerCase();
    sanitized[key] = SENSITIVE_HEADER_KEYS.has(lowerKey) ? '[REDACTED]' : headersInput[key];
  }
...
function sanitizeBody(body, contentType) {
  ...
  if (normalizedContentType.includes('application/x-www-form-urlencoded')) {
    ...
    for (const [key, value] of params.entries()) {
      sanitized[key] = redactIfSensitive(key, value);
    }
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object') {
        ...
        for (const key of Object.keys(parsed)) {
          sanitized[key] = redactIfSensitive(key, parsed[key]);
        }
        return sanitized;
      }
    } catch {
...
export function installSafeApiLogger() {
  if (globalThis[FETCH_LOGGER_FLAG]) {
    return;
  }
  ...
  globalThis.fetch = async (input, init = {}) => {
    ...
    const contentType = headers && typeof headers === 'object' ? headers['Content-Type'] || headers['content-type'] : undefined;
