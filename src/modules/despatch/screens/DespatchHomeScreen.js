...
import { getDashboardSummary } from '../services/despatch';

const WEEK_DAYS = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'];
const THEME = '#059669';
const THEME_BG = '#f0fdf4';
const THEME_BG_PAGE = '#f0fdf4';
...
export default function DespatchHomeScreen({ navigation }) {
  const [displayedDate, setDisplayedDate] = useState(new Date());
  const [selectedDay, setSelectedDay] = useState(new Date().getDate());
  const [summaryData, setSummaryData] = useState({ total: 0, pending: 0, despatched: 0 });
  ...
  const loadSummary = async () => {
    try {
      ...
      console.error('Error loading despatch summary:', error);
    }
  };
        ...
        <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
          <View style={styles.heroCard}>
            ...
            <View style={styles.heroTitleRow}>
              <Text style={styles.heroTitle}>Despatch</Text>
            </View>
...
const styles = StyleSheet.create({
  ...
  calendarSection: {
    backgroundColor: '#f7fffe', borderRadius: 28, padding: 18, marginBottom: 20,
