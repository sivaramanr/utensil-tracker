...
export async function ensureUtensilTypesTable() {
  ...
  await db.execAsync(`
    CREATE TABLE IF NOT EXISTS ${UTENSIL_TYPES_TABLE} (
      id TEXT PRIMARY KEY,
      unitId TEXT, accountId TEXT, parentId TEXT, parentName TEXT,
      genericType TEXT, name TEXT, type TEXT, value TEXT,
      isActive BOOLEAN, description TEXT, "order" INTEGER,
      createdAt INTEGER, createdBy TEXT, updatedAt INTEGER, updatedBy TEXT
    );
  `);
}
...
export async function getUtensilTypeItemGroups(utensilTypeId) {
  const db = await getDatabase();
  const rows = await db.getAllAsync(
    `SELECT utensilTypeId, itemGroupId FROM ${UTENSIL_TYPE_ITEM_GROUP_TABLE}
     WHERE utensilTypeId = ? ORDER BY itemGroupId ASC;`,
    utensilTypeId
  );
  return rows.map((row) => ({
...
function normalizeItemGroupMappings(payload) {
  return normalizeApiList(payload).flatMap((item) => {
    const utensilTypeId = item?.utensilTypeId != null ? String(item.utensilTypeId) : null;
    if (!utensilTypeId) return [];
    return (Array.isArray(item?.itemGroupIds) ? item.itemGroupIds : [])
      .map((id) => ({ utensilTypeId, itemGroupId: id != null ? String(id) : null }))
      .filter((m) => m.itemGroupId);
  });
}

async function fetchUtensilTypesFromApi() {
  const [typesPayload, mappingsPayload] = await Promise.all([
    apiGet('/genericmodel', {
      params: { genericType: 'UTENSIL_TYPE', page: '0', size: '20' },
      label: 'Utensil Types API',
    }),
    apiGet('/utensiltype', {
      params: { page: '0', size: '1000' },
      label: 'Utensil Type Item Group API',
    }),
  ]);

  const types = normalizeTypes(typesPayload);
  const mappings = normalizeItemGroupMappings(mappingsPayload);
  await replaceUtensilTypeItemGroups(mappings);
  return types;
}
...
async function replaceUtensilTypeItemGroups(list) {
  ...
  await db.withTransactionAsync(async () => {
    ...
    for (const item of list) {
      await db.runAsync(
        `INSERT INTO ${UTENSIL_TYPE_ITEM_GROUP_TABLE} (utensilTypeId, itemGroupId) VALUES (?, ?);`,
        item.utensilTypeId, item.itemGroupId
      );
    }
  });
}
      ...
      await db.runAsync(
        `INSERT INTO ${UTENSIL_TYPES_TABLE}
         (id, unitId, accountId, parentId, parentName, genericType, name,
          type, value, isActive, description, "order", createdAt, createdBy, updatedAt, updatedBy)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);`,
        item.id, item.unitId, item.accountId, item.parentId, item.parentName,
        item.genericType, item.name, item.type, item.value,
        item.isActive ? 1 : 0, item.description, item.order,
...
async function ensureFreshData() {
  await Promise.all([ensureUtensilTypesTable(), ensureUtensilTypeItemGroupTable()]);
  const [typeCount, igCount] = await Promise.all([getUtensilTypesCount(), getUtensilTypeItemGroupCount()]);
  if (typeCount === 0 || igCount === 0) {
    await replaceUtensilTypes(await fetchUtensilTypesFromApi());
  }
}

export async function loadUtensilTypesWithInitialSync() {
  await ensureFreshData();
  const local = await getUtensilTypes();
