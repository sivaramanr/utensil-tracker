...
import { loadSessionsWithInitialSync, refreshSessionsFromApi } from '../services/sessions';
...
export default function SessionScreen({ navigation, route }) {
  ...
  const canSkipSession = route?.params?.canSkipSession ?? false;
      ...
      >
        ...
        <View style={styles.sessionContent}>
          <View style={styles.sessionTitleRow}>
            <Text style={styles.sessionCardName} numberOfLines={1}>{item.name}</Text>
            <View
              style={[
        ...
        <View style={styles.heroTopRow}>
          <View style={styles.heroBrandWrap}>
            ...
            <View style={styles.heroTextWrap}>
              <Text style={[styles.heroEyebrow, { color: themeColor }]}>{displayedDate}</Text>
              <Text style={styles.title}>Sessions</Text>
            </View>
          </View>
          <View style={[styles.heroBadge, { backgroundColor: themeBg }]}>
            <Ionicons name="calendar-outline" size={14} color={themeColor} />
            <Text style={[styles.heroBadgeText, { color: themeColor }]}>{sessions.length}</Text>
          </View>
        </View>
        {currentDate && (
      ...
      {loading ? (
        ...
        <FlatList
          ...
          ListFooterComponent={
            canSkipSession ? (
              <Pressable
                style={[styles.skipCard, { borderColor: themeColor }]}
                onPress={() =>
                  navigation.navigate('Customers', {
                    sessionId: null,
                    sessionName: null,
              ...
              >
                <View style={[styles.skipIconWrap, { backgroundColor: themeBg }]}>
                  <Ionicons name="git-branch-outline" size={20} color={themeColor} />
                </View>
                <View style={styles.skipTextWrap}>
                  <Text style={[styles.skipTitle, { color: themeColor }]}>Skip Session</Text>
                  <Text style={styles.skipCaption}>Browse all customers for this date</Text>
                </View>
...
const styles = StyleSheet.create({
  ...
  sessionStatusBadgeActive: { backgroundColor: '#dcfce7' },
  sessionStatusBadgeInactive: { backgroundColor: '#e5e7eb' },
  sessionStatusText: { fontSize: 11, fontWeight: '700' },
  sessionStatusTextActive: { color: '#166534' },
  sessionStatusTextInactive: { color: '#475569' },
  sessionCaption: {
    fontSize: 13,
    color: '#64748b',
    marginTop: 8,
  ...
  skipTextWrap: { flex: 1, gap: 2 },
  skipTitle: { fontSize: 15, fontWeight: '700' },
  skipCaption: { fontSize: 12, color: '#9ca3af' },
  toastContainer: {
    position: 'absolute',
    left: 16,
    right: 16,
